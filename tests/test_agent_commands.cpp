...
public:
    ...
    quantclaw::ChatCompletionResponse ChatCompletion(
        const quantclaw::ChatCompletionRequest& /*req*/) override {
    ...
    std::string GetProviderName() const override { return "agent-cmd-mock"; }
...
protected:
    void SetUp() override {
        ...
        session_manager_ = std::make_shared<quantclaw::SessionManager>(
            sessions_dir_, logger_);
...
TEST_F(AgentCommandsIntegrationTest, AuthTokenMismatchReturnsError) {
    ...
    server_ = std::make_unique<quantclaw::gateway::GatewayServer>(auth_port, logger_);
    server_->SetAuth(auth_config.gateway.auth.mode, auth_config.gateway.auth.token);
    quantclaw::gateway::register_rpc_handlers(
        *server_, session_manager_, agent_loop_, prompt_builder_,
        tool_registry_, auth_config, logger_);
    server_->Start();
    std::this_thread::sleep_for(std::chrono::milliseconds(200));
