...
std::vector<std::string> DefaultScopes(Role role) {
  switch (role) {
    case Role::kOperator:
      return {scopes::kOperatorRead, scopes::kOperatorWrite,
...
void RBACChecker::init_default_rules() {
  ...
  for (const char* method :
       {m::kGatewayHealth,    m::kGatewayStatus,     m::kConfigGet,
        m::kSessionsList,     m::kSessionsHistory,   m::kChannelsList,
        m::kChannelsStatus,   m::kOcHealth,          m::kOcStatus,
        m::kOcModelsList,     m::kOcSessionsPreview, m::kOcChatHistory,
        m::kSkillsStatus,     m::kExecApprovals,     m::kCronList,
        m::kCronRuns,         m::kMemoryStatus,      m::kMemorySearch,
        m::kPluginsList,      m::kPluginsTools,      m::kPluginsServices,
        m::kPluginsProviders, m::kPluginsCommands,   m::kPluginsGateway,
        m::kQueueStatus}) {
    method_scopes_[method] = {scopes::kOperatorRead, scopes::kOperatorWrite,
                              scopes::kOperatorAdmin, scopes::kNodeRead,
                              scopes::kNodeExecute};
  }

  // Write methods: operator.write or operator.admin
  for (const char* method :
       {m::kConfigSet, m::kConfigReload, m::kSessionsDelete, m::kSessionsReset,
        m::kSessionsPatch, m::kSessionsCompact, m::kModelsSet,
        m::kSkillsInstall, m::kCronAdd, m::kCronRemove, m::kCronUpdate,
        m::kCronRun, m::kExecApprovalReq, m::kPluginsCallTool,
        m::kQueueConfigure, m::kQueueCancel, m::kQueueAbort}) {
    method_scopes_[method] = {scopes::kOperatorWrite, scopes::kOperatorAdmin};
  }

  // Agent execution: operator.write or node.execute
  for (const char* method : {m::kAgentRequest, m::kAgentStop, m::kOcChatSend,
                             m::kOcChatAbort, m::kChainExecute}) {
    method_scopes_[method] = {scopes::kOperatorWrite, scopes::kOperatorAdmin,
                              scopes::kNodeExecute};
  }
  ...
  const std::unordered_set<std::string> read_scopes{
      scopes::kOperatorRead, scopes::kOperatorWrite, scopes::kOperatorAdmin,
      scopes::kNodeRead, scopes::kNodeExecute};
  for (const auto& method : std::vector<std::string>{
           "agent.identity.get", "node.list", "device.pair.list", "logs.tail",
           "usage.cost", "sessions.usage", "sessions.usage.timeseries",
           "sessions.usage.logs", "cron.status", "config.schema",
           "agents.list"}) {
...
bool RBACChecker::IsAllowed(
    const std::string& method, const std::string& /*role*/,
...
RBACChecker::RequiredScopes(const std::string& method) const {
  auto it = method_scopes_.find(method);
  if (it == method_scopes_.end())
    return {scopes::kOperatorAdmin};
