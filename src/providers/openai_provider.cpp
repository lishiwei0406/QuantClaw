...
std::string message_text_content(const Message& msg) {
  std::string text;
...
std::string message_reasoning_content(const Message& msg) {
...
std::string json_text_content_or_empty(const nlohmann::json& value) {
  if (value.is_null()) {
    return "";
  }
  if (value.is_string()) {
  ...
  if (value.is_array()) {
    ...
    for (const auto& item : value) {
      if (item.is_string()) {
        ...
        text += detail::json_nullable_string_or_empty(item, "text");
      }
    }
    return text;
  }
  if (value.is_object()) {
    return detail::json_nullable_string_or_empty(value, "text");
  }
  return "";
}
...
serialize_messages_to_openai(const std::vector<Message>& messages,
  ...
  for (const auto& msg : messages) {
    ...
    if (has_tool_result) {
      ...
      // Older transcripts may contain assistant tool-call turns from before
      // reasoning blocks were persisted. When thinking is enabled, keep the
      // field present so OpenAI-compatible APIs accept the replayed history.
...
std::string SummarizeErrorBodyForLog(std::string_view body) {
  constexpr size_t kMaxLoggedChars = 256;
  std::string result;
  result.reserve(std::min(body.size(), kMaxLoggedChars));
  ...
  for (char ch : body) {
    ...
    if (result.size() >= kMaxLoggedChars) {
      break;
    }
    result.push_back(ch);
...
static size_t StreamWriteCallback(void* contents, size_t size, size_t nmemb,
  ...
  while ((pos = ctx->buffer.find('\n')) != std::string::npos) {
    ...
    if (delta.contains("content") && !delta["content"].is_null()) {
      ...
      resp.content = json_text_content_or_empty(delta["content"]);
    ...
    if (delta.contains("reasoning_content") &&
      ...
      resp.reasoning_content =
          json_text_content_or_empty(delta["reasoning_content"]);
    ...
    if (delta.contains("reasoning") && !delta["reasoning"].is_null()) {
      ChatCompletionResponse resp;
      resp.reasoning_content = json_text_content_or_empty(delta["reasoning"]);
      if (!resp.reasoning_content.empty()) {
        ctx->callback(resp);
      }
    }
